...
(function (w) {
  ...
  /* 客戶版是獨立 build 的檔案（內部資料根本不在 HTML 裡），並且鎖死身分不給切換 */
  function customerOnly() { return !!SEED.customerOnly; }
  function mode() { if (customerOnly()) return 'customer'; try { return localStorage.getItem(LS_MODE) || 'aces'; } catch (e) { return 'aces'; } }
  function setMode(m) { if (customerOnly()) return; try { localStorage.setItem(LS_MODE, m === 'customer' ? 'customer' : 'aces'); } catch (e) {} }
